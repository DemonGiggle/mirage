...
- On a rootless host, Mirage uses `mmdebstrap --mode=unshare --format=tar`,
  ...
  manages at runtime. For trees generated without sudo, Mirage then enters a
  temporary mapped user namespace and shifts the normalized tree to the
  caller's subordinate root ID.
...
`newuidmap` and `newgidmap` helpers. Runtime keep-ID mapping also requires
util-linux 2.39 or newer for multi-range `unshare --map-users/--map-groups`.
...
Rootless trees generated with `rootfs init --sudo` also retain the legacy map.
Changing an inode's owner clears its setuid bit, and an unprivileged namespace
cannot restore that bit on the host-mounted filesystem. Keeping the legacy
ownership therefore preserves the existing guest sudo feature; keep-ID applies
to rootless trees generated without `--sudo`.
...
directly. A rootless archive bootstrap retains caller ownership and the setuid
mode; the legacy runtime map presents that caller ownership as guest root.
