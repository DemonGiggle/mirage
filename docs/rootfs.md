...
The `/usr/bin/sudo` generated by `rootfs init --sudo` works with both bootstrap strategies. A
privileged directory bootstrap preserves Debian's root ownership directly. A
rootless archive bootstrap maps the invoking host user to guest root and keeps
the setuid mode, so Mirage sees the same root-owned setuid binary after entering
the runtime user namespace.
